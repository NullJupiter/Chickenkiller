...
namespace Asylum {
	...
	void Renderer::Init()
	{
		...
		sRendererData.DefaultRectVertices[0] = { -0.5f, -0.5f, 0.0f, 1.0f };
		sRendererData.DefaultRectVertices[1] = {  0.5f, -0.5f, 0.0f, 1.0f };
		sRendererData.DefaultRectVertices[2] = {  0.5f,  0.5f, 0.0f, 1.0f };
		sRendererData.DefaultRectVertices[3] = { -0.5f,  0.5f, 0.0f, 1.0f };

		// initialize standard texture coordinates
		sRendererData.DefaultRectTexCoords[0] = { 0.0f, 0.0f };
	...
	float Renderer::TextureChecks(uint32_t textureID)
	{
	...
	void Renderer::AssignVerticesToBuffer(float textureIndex, const glm::vec2& position, const glm::vec2& size, const glm::vec4& tint, const std::array<glm::vec2, 4>& textureCoords, float angle)
	{
		...
		sRendererData.RectBufferPtr->Position = glm::vec3(transform * sRendererData.DefaultRectVertices[0]);
		sRendererData.RectBufferPtr->Color = rectTint;
		sRendererData.RectBufferPtr->TexCoord = textureCoords[0];
		sRendererData.RectBufferPtr->TexSlot = textureIndex;
		sRendererData.RectBufferPtr++;

		sRendererData.RectBufferPtr->Position = glm::vec3(transform * sRendererData.DefaultRectVertices[1]);
		sRendererData.RectBufferPtr->Color = rectTint;
		sRendererData.RectBufferPtr->TexCoord = textureCoords[1];
		sRendererData.RectBufferPtr->TexSlot = textureIndex;
		sRendererData.RectBufferPtr++;

		sRendererData.RectBufferPtr->Position = glm::vec3(transform * sRendererData.DefaultRectVertices[2]);
		sRendererData.RectBufferPtr->Color = rectTint;
		sRendererData.RectBufferPtr->TexCoord = textureCoords[2];
		sRendererData.RectBufferPtr->TexSlot = textureIndex;
		sRendererData.RectBufferPtr++;

		sRendererData.RectBufferPtr->Position = glm::vec3(transform * sRendererData.DefaultRectVertices[3]);
		sRendererData.RectBufferPtr->Color = rectTint;
		sRendererData.RectBufferPtr->TexCoord = textureCoords[3];
		sRendererData.RectBufferPtr->TexSlot = textureIndex;
		sRendererData.RectBufferPtr++;
	...
	void Renderer::DrawRectangle(const glm::vec2& position, const glm::vec2& size, const glm::vec4& color, float angle)
	{
		FullBufferCheck();

		// texture index is set to zero because for colored rects the white texture is being used
		AssignVerticesToBuffer(0.0f, position, size, color, sRendererData.DefaultRectTexCoords, angle);
	}

	void Renderer::DrawRectangle(const glm::vec2& position, const glm::vec2& size, const Ref<Texture>& texture, const glm::vec4& tint, float angle)
	{
		FullBufferCheck();
		AssignVerticesToBuffer(TextureChecks(texture->GetID()), position, size, tint, sRendererData.DefaultRectTexCoords, angle);
	}
