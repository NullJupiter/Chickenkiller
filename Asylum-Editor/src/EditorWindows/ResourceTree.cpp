...
void ResourceTree::OnUpdate(float dt)
{
	...
	{
		const std::unordered_map<std::string, Asylum::Ref<Asylum::Animation>>& animations = Asylum::ResourceManager::GetAllAnimationData();
		ImGui::Columns(4);
		ImGui::Separator();
		uint32_t i = 0;
		for (auto& animationData : animations)
		{
			animationData.second->OnUpdate(dt);
			if (i > 0 && i % 4 == 0) ImGui::Separator();
			const std::array<glm::vec2, 4>& currentTexCoords = animationData.second->GetCurrentTextureCoords();
			if (ImGui::ImageButton((void*)animationData.second->GetAnimationSheet()->GetID(), ImVec2(ImGui::GetContentRegionAvail().x, ImGui::GetContentRegionAvail().x), ImVec2(currentTexCoords[2].x, currentTexCoords[2].y), ImVec2(currentTexCoords[0].x, currentTexCoords[0].y), 0))
			{}
			ImGui::TextWrapped(animationData.first.c_str());
